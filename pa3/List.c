...
void deleteBack(List L){
   ...
   Node N = L->back; //Placeholder to free the back later
...
void set(List L, long x){
	if( L==NULL ){
   	...
   	if(length(L) <= 0){
   		printf("List Error: calling set() on on empty list\n");
     	exit(1);
   	}
   	...
   	int tempInd = index(L); //Save the index we want to end up on

		insertBefore(L, x); //Cursor is defined so we can insert the node with
		delete(L);          //value we want before it. Delete the node we dont want
		moveFront(L);       //Move the cursor to the front to start iterating
   	
      while(index(L) != tempInd){ //Iterate thru the list to set the cursor
         moveNext(L);             //back to what it was.
      }
